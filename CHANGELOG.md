...
### 修复（前端显示 Bug 修复）
  - 修复聊天窗口和 SubAgent Monitor 中工具卡片显示不完整的问题，文件路径和工具参数不再容易变成 `?`。
  - 修复工具参数已经显示出来、卡片却还写着“正在生成参数”的问题，让状态提示更符合实际进度。
  - 修复 SubAgent Monitor 卡住后原地重置容易变空白的问题，重连时会尽量保留旧内容并显示更清楚的等待状态。
  - 修复 SubAgent 运行中卡片被大段原始提示词或 JSON 撑开的情况，让主界面优先显示简洁的任务说明。
  - 优化等待、加载和转圈动画的显示稳定性，减少界面闪烁、转圈不动和加载提示位置不一致的问题。

### 发布整理
  - 根扩展包、前端包、lockfile 和发布说明统一更新到 `1.2.7`。
  - 仍需后续单独处理真实外部模型长任务验证、Cursor 命令行安装验证、依赖安全升级和前端包体优化。
