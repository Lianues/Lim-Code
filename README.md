...
- 修复主聊天长回复流式正文闪烁：稳定 text / thought 渲染块身份，并让 Markdown streaming 首帧即时显示、持续输出有最大等待上限。
- 恢复 SubAgent Monitor 实时输出：Monitor 现在接收轻量 text / thought / functionCall delta，同时仍通过窗口接口按需校准完整 transcript。
- 修复 SubAgent Monitor 实时工具卡参数显示，`read_file` 等工具在 live 状态下不再因缺少 args 临时显示 `?`。
- 1.2.1 尚未正式发布；1.2.2 吸收原 1.2.1 预发布整理内容，并追加本次流式稳定性修复。
...
当前说明：版本已更新为 `1.2.2`；1.2.1 未正式发布，其预发布整理内容已并入本版本。本次发布重点是主聊天流式正文稳定、SubAgent Monitor 实时输出恢复、实时工具卡参数显示修复，以及对应的渲染一致性改进。发布材料只描述结构性优化和用户可见修复，不声明未经量化基线验证的具体性能收益数字。
