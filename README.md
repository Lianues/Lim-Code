...
- 优化对话窗口与后台标签页的窗口化状态处理，减少大对话场景下的全量快照和重复序列化。
- 重构 SubAgent Monitor 前端数据流，使用轻量运行清单和按需 transcript window，降低大输出监控面板打开与实时更新压力。
- 整合 1.2.0 未公开发布阶段的架构整理、Webview 路由修复和 Anthropic 流式工具卡片修复。
...
当前说明：版本已更新为 `1.2.1`；本次发布整合 1.2.0 未公开发布阶段的架构整理，并追加窗口序列化与 SubAgent Monitor 前端架构性能优化。发布材料只描述结构性优化，不声明未经量化基线验证的具体性能收益数字。
