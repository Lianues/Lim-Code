...
export interface Content {
     ...
     * 第一个流式块时间戳（毫秒）
     *
     * 修改原因：Token 速率不再使用首块到末块窗口，否则上游攒包会导致速度畸高。
     * 修改方式：该字段仅表示首块到达时间，用于展示或诊断首块延迟。
     * 修改目的：保留首块延迟诊断能力，同时避免误用为速率分母。
     */
     ...
     * 流式响应完整持续时间（毫秒）
     *
     * 修改原因：旧语义只统计首块到末块，无法覆盖等待首块前 AI 已经消耗的时间。
     * 修改方式：由 StreamAccumulator 计算为请求开始到最后一个流式块；旧前端缺少 responseDuration 时可作为兼容回退。
     * 修改目的：让 token 速率基于完整请求耗时，避免 SSE 一次性输出时出现畸高速度。
     */
